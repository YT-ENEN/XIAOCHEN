<?php
// ==========================================
// 1. 設定區：填入你的 API Key 與「須知資料夾」網址
// ==========================================
$api_key = "AIzaSyB0pjUimtoomamQ9BVPGwIB-36VNFlVlds";

// ▼ 這是你指定的須知資料夾網址 ▼
$master_folder_url = "https://drive.google.com/drive/folders/16CFWtuK-c16xAoU_1EIG3vos-mchBC9D"; 
...
if (!empty($master_folder_id)) {
    // 撈取該資料夾底下的所有圖片，並依名稱 (name) 排序，確保 01, 02 順序正確
    $query = "'" . $master_folder_id . "' in parents and mimeType contains 'image/' and trashed = false";
    ...
    if (isset($data['error'])) {
        ...
        foreach ($data['files'] as $file) {
            // 使用官方 Thumbnail API 繞過防盜鏈鎖定
            $images[] = "https://drive.google.com/thumbnail?id=" . $file['id'] . "&sz=w1000";
        }
    }
} else {
    $drive_api_error = "請在程式碼頂部填入有效的資料夾網址。";
}

// 網站基本設定
$site_title = "XIAOCHEN | 購買須知";
...
<head>
    ...
    <style>
        ...
        .modal-content { width: 90%; max-width: 600px; max-height: 85vh; overflow-y: auto; background: var(--glass); border: 1px solid var(--accent); border-radius: 15px; padding: 20px; text-align: center; }
        .modal-content::-webkit-scrollbar { width: 8px; }
        .modal-content::-webkit-scrollbar-thumb { background: var(--accent); border-radius: 4px; }
        
        .close-btn { position: absolute; top: 20px; right: 40px; font-size: 3rem; color: #fff; cursor: pointer; transition: 0.3s; z-index: 2100; }
        .close-btn:hover { color: var(--accent); }
        
        /* 彈出後的放大圖片 */
        .modal-focused-img { width: 100%; height: auto; border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); }
...
<body>
    ...
    <div class="content-wrapper">
        <nav>
            ...
            <div class="nav-links">
                ...
                <a href="#accounts-grid">購買須知</a>
                <a href="三角洲.php">帳號一覽</a>
            </div>
        ...
        <div id="accounts-grid" class="account-grid">
            <?php if (!empty($drive_api_error)): ?>
                <div style="grid-column: 1 / -1; background: rgba(255, 50, 50, 0.15); border: 1px solid #ff4d4d; border-radius: 15px; padding: 30px; color: #ff4d4d; font-family: monospace;">
                    <h3 style="margin-top: 0;">⚠️ Google Drive API 連線失敗 (讀取須知資料夾)</h3>
                    <p style="font-size: 1.1rem; line-height: 1.5;"><?php echo $drive_api_error; ?></p>
                </div>
            <?php elseif (empty($images)): ?>
                <div style="text-align: center; grid-column: 1 / -1;">
                    <p style="color: #aaa;">資料夾中目前沒有任何須知圖片。</p>
                </div>
            <?php else: ?>
                <?php foreach ($images as $img_url): ?>
                    <div class="acc-card" onclick="zoomImage('<?php echo $img_url; ?>')">
                        <img src="<?php echo $img_url; ?>" alt="Notice Image" class="acc-cover" onerror="this.src='https://via.placeholder.com/300x415/151515/00f2ff?text=Image+Error'">
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
    ...
    <div id="img-modal" onclick="closeModal()">
        ...
        <div class="modal-content" onclick="event.stopPropagation()">
            <img src="" id="modal-focused-target" class="modal-focused-img" alt="Focused Notice">
        </div>
    </div>
    ...
    <script>
        ...
        const modalImg = document.getElementById('modal-focused-target');
